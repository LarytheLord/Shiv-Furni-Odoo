...
router.get(
  ...
  param('id').isUUID(),
  validateRequest,
  budgetController.getMetrics,
);
router.get(
  '/:id/export-pdf',
  param('id').isUUID(),
  validateRequest,
  budgetController.exportPdf,
);
...
router.post(
  ...
  adminOnly,
  param('id').isUUID(),
  [body('analyticalAccountId').isUUID(), body('plannedAmount').isNumeric()],
  validateRequest,
...
router.patch(
  '/:id/lines/:lineId',
  adminOnly,
  [param('id').isUUID(), param('lineId').isUUID()],
  validateRequest,
