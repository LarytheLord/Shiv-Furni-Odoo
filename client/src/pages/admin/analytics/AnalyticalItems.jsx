...
export default function AnalyticalItems() {
    ...
    const fetchItems = async () => {
        ...
        try {
            ...
            let endpoint = '';
            if (type === 'INCOME') endpoint = '/customer-invoices'; // This lists Headers, not Lines.
    ...
    const columns = [
        { header: 'Number', accessor: type === 'INCOME' ? 'invoiceNumber' : 'billNumber', width: '20%' },
        { header: 'Date', accessor: type === 'INCOME' ? 'invoiceDate' : 'billDate', render: r => new Date(r[type === 'INCOME' ? 'invoiceDate' : 'billDate']).toLocaleDateString() },
        { header: 'Status', accessor: 'status' },
        { header: 'Total', accessor: 'total', render: r => `₹${Number(r.total).toLocaleString()}` }
    ];
