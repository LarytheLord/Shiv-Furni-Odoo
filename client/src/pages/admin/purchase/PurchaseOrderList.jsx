import React, { useState } from 'react';
import { useNavigate } from 'react-router-dom';
import ListView from '../../../components/ui/ListView';

export default function PurchaseOrderList() {
    const navigate = useNavigate();

    const [data] = useState([
        { id: 1, name: 'PO0001', vendor: 'Azure Interior', date: '2026-01-15', total: 16350, status: 'Confirm' },
        { id: 2, name: 'PO0002', vendor: 'Deco Addict', date: '2026-01-20', total: 4500, status: 'Draft' },
    ]);

    const columns = [
        {
            header: 'PO No.', accessor: 'name', render: (row) => (
                <span
                    className="font-medium text-slate-700 cursor-pointer hover:text-accent-600"
                    onClick={() => navigate(`/admin/purchase-orders/${row.id}`)}
                >
                    {row.name}
                </span>
            )
        },
        { header: 'Vendor', accessor: 'vendor' },
        { header: 'Order Date', accessor: 'date' },
        { header: 'Total', accessor: 'total', render: (row) => row.total.toLocaleString() },
        {
            header: 'Status', accessor: 'status', render: (row) => (
                <span className={`px-2 py-1 rounded text-xs font-semibold ${row.status === 'Confirm' ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-700'}`}>
                    {row.status}
                </span>
            )
        }
    ];

    return (
        <ListView
            title="Purchase Orders"
            columns={columns}
            data={data}
            pagination={{ page: 1, limit: 10, total: 2 }}
            onCreate={() => navigate('/admin/purchase-orders/new')}
        />
    );
}
