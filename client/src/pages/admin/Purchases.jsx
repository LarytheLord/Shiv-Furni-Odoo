...
import { Plus, Search, MoreHorizontal, Truck, Calendar, User } from 'lucide-react';
...
export default function Purchases() {
  ...
  const fetchOrders = async () => {
    try { const { data } = await api.get('/purchase-orders'); setOrders(data.orders || []); }
    catch (err) { console.error(err); }
    finally { setLoading(false); }
  };

  const filtered = orders.filter(o => o.reference?.toLowerCase().includes(searchQuery.toLowerCase()) || o.vendor?.name?.toLowerCase().includes(searchQuery.toLowerCase()));
  ...
  return (
    <div className="page-container">
      <div className="page-header"><div><h1>Purchase Orders</h1><p>Manage vendor orders and procurement</p></div><button className="btn-primary"><Plus size={18} /><span>New Purchase</span></button></div>
      <div className="filters-bar"><div className="search-box"><Search size={18} /><input type="text" placeholder="Search purchases..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} /></div></div>
      <div className="card">
        ...
        : filtered.length === 0 ? <div className="empty-state"><div className="empty-icon"><Truck size={32} /></div><h3>No purchase orders found</h3><p>Create your first purchase order</p></div>
        : <table className="table"><thead><tr><th>Reference</th><th>Vendor</th><th>Date</th><th>Amount</th><th>Status</th><th></th></tr></thead><tbody>
            {filtered.map((o) => {
              ...
              return (
                <tr key={o.id}>
                  <td><div className="cell-with-avatar"><div className="avatar purple"><Truck size={18} /></div><span className="primary-text">{o.reference}</span></div></td>
                  <td><div className="cell-with-icon"><User size={14} /><span>{o.vendor?.name || '—'}</span></div></td>
                  <td><div className="cell-with-icon"><Calendar size={14} /><span>{new Date(o.orderDate).toLocaleDateString()}</span></div></td>
                  <td><span className="amount expense">{fmt(o.totalAmount)}</span></td>
