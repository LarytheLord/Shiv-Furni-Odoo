...
export default function Purchases() {
  ...
  const [formData, setFormData] = useState({ vendor_id: '', order_date: '', lines: [] });
  ...
  const fetchData = async () => {
    try {
      const [ordersRes, vendorsRes, productsRes] = await Promise.all([
        ...
        api.get('/contacts'), // We should filter by type='vendor' ideally, but backend returns all. Frontend filter ok.
        api.get('/products')
      ]);
  ...
  const addLine = () => {
    setFormData({
      ...formData,
      lines: [...formData.lines, { product_id: '', quantity: 1, unit_price: 0, analytical_account_id: null }] // account auto-filled by backend mostly
    });
  };
  ...
  const updateLine = (index, field, value) => {
    ...
    if (field === 'product_id') {
      ...
      if (product) {
        newLines[index].unit_price = product.cost_price; // default to cost for PO
      }
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setShowModal(false);
      setFormData({ vendor_id: '', order_date: '', lines: [] });
      fetchData();
    } catch (err) {
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-6">
        ...
        <button
          onClick={() => {
              setFormData({ vendor_id: '', order_date: new Date().toISOString().split('T')[0], lines: [] });
              setShowModal(true);
          }}
      ...
      <div className="bg-white shadow-md rounded-lg overflow-hidden">
        <table className="min-w-full divide-y divide-gray-200">
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {orders.map((order) => (
              <tr key={order.id}>
                <td className="px-6 py-4 whitespace-nowrap">PO-{order.id}</td>
                <td className="px-6 py-4 whitespace-nowrap">{order.vendor_name}</td>
                ...
                <td className="px-6 py-4 whitespace-nowrap">
                        ...
                        <button 
                            ...
                            onClick={async () => {
                                if(window.confirm('Confirm this PO?')) {
                                    await api.post(`/purchases/orders/${order.id}/confirm`);
                                    fetchData();
                                }
                            }}
                        >
                    ...
                    {order.state === 'confirmed' && (
                        <button 
                            ...
                            onClick={async () => {
                                if(window.confirm('Create Bill from this PO?')) {
                                    await api.post(`/purchases/bills/from-po`, { purchase_order_id: order.id });
                                    alert('Bill created!');
                                    // Navigate to bills?
                                }
                            }}
