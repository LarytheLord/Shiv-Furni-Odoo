...
export default function BudgetList() {
  ...
  const columns = [
    ...
    },
    {
      ...
      accessor: 'dateFrom',
      width: '15%',
      render: (row) => new Date(row.dateFrom).toLocaleDateString(),
    },
    {
      header: 'End Date',
      accessor: 'dateTo',
      width: '15%',
      render: (row) => new Date(row.dateTo).toLocaleDateString(),
    },
    {
      header: 'Status',
      accessor: 'status',
      ...
      render: (row) => {
        ...
        return (
          <span
            className={`px-2 py-1 rounded-full text-xs font-semibold ${map[row.status] || 'bg-gray-100'}`}
          >
            {row.status}
          </span>
        );
              ...
              <div className='grid grid-cols-2 gap-2 mt-2 text-center'>
                <div className='bg-blue-50 p-2 rounded'>
                  ...
                  <div className='text-sm font-bold text-slate-700'>
                    ₹{chartPopover.data.practical?.toLocaleString() ?? 0}
                  </div>
                </div>
                <div className='bg-red-50 p-2 rounded'>
                  ...
                  <div className='text-sm font-bold text-slate-700'>
                    ₹{chartPopover.data.remaining?.toLocaleString() ?? 0}
                  </div>
                </div>
              </div>
            </div>
