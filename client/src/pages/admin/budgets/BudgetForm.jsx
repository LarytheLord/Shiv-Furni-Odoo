...
export default function BudgetForm() {
  ...
  const fetchBudget = async () => {
    ...
    try {
      ...
      const lines = budget.budgetLines.map((line) => {
        const metric =
          metricsLines.find((m) => m.id === line.id)?.metrics || {};
        return {
          id: line.id,
    ...
    try {
      ...
      const payload = {
        ...
        lines: formData.lines.map((line) => ({
          id:
            line.id && !String(line.id).startsWith("temp-")
              ? line.id
              : undefined,
          analyticalAccountId: line.analyticalAccountId,
          type: line.type,
  ...
  const handleRevise = async () => {
    if (!confirm("Set this budget back to draft mode for editing?")) return;
    try {
      await api.post(`/budgets/${id}/revise`);
      fetchBudget(); // Refresh to get updated status (now DRAFT)
    } catch (err) {
      console.error(err);
  ...
  const handleRemoveLine = (lineId) => {
    setFormData((prev) => ({
      ...
      linesToDelete: !String(lineId).startsWith("temp-")
        ? [...prev.linesToDelete, lineId]
        : prev.linesToDelete,
    }));
  };
        ...
        <>
          {formData.status === "DRAFT" && !isNew && (
            <button
              ...
              className="btn-erp btn-erp-primary bg-emerald-600 hover:bg-emerald-700 text-white"
            >
              Confirm
            </button>
          )}
          ...
          {!isNew && (
            <button
              ...
              className="btn-erp btn-erp-secondary"
              title="Compute Achieved Amounts"
            >
                ...
                >
                  ...
                  <td className="py-2 px-4">
                    <select
                      ...
                      value={line.type}
                      onChange={(e) =>
                        handleLineChange(line.id, "type", e.target.value)
                      }
                      disabled={isReadOnly}
                    >
                    ...
                    <div className="flex items-center justify-end gap-2 group">
                      ...
                      {!isNew && line.achievedAmount !== 0 && (
                        <button
                          onClick={() =>
                            navigate(
                              `/admin/analytics/items?analyticId=${line.analyticalAccountId}&type=${line.type}&dateFrom=${formData.dateFrom}&dateTo=${formData.dateTo}`,
                            )
                          }
                  ...
                  <td className="py-2 px-4 text-right">
                    <span
                      className={`font-medium ${line.achievementPercent >= 100 ? "text-red-500" : "text-slate-700"}`}
                    >
