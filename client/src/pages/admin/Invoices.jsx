...
export default function Invoices() {
  ...
  const filtered = invoices.filter(i => i.reference?.toLowerCase().includes(searchQuery.toLowerCase()) || i.customer?.name?.toLowerCase().includes(searchQuery.toLowerCase()));
  ...
  return (
    <div className="page-container">
      <div className="page-header"><div><h1>Invoices</h1><p>Manage customer invoices and track payments</p></div><button className="btn-primary"><Plus size={18} /><span>New Invoice</span></button></div>
      <div className="filters-bar"><div className="search-box"><Search size={18} /><input type="text" placeholder="Search invoices..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} /></div></div>
      <div className="card">
        ...
        : filtered.length === 0 ? <div className="empty-state"><div className="empty-icon"><FileText size={32} /></div><h3>No invoices found</h3><p>Create your first invoice</p></div>
        : <table className="table"><thead><tr><th>Reference</th><th>Customer</th><th>Date</th><th>Amount</th><th>Status</th><th></th></tr></thead><tbody>
            {filtered.map((i) => {
              ...
              return (
                <tr key={i.id}>
                  <td><div className="cell-with-avatar"><div className="avatar green"><FileText size={18} /></div><span className="primary-text">{i.reference}</span></div></td>
                  <td><div className="cell-with-icon"><User size={14} /><span>{i.customer?.name || '—'}</span></div></td>
                  <td><div className="cell-with-icon"><Calendar size={14} /><span>{new Date(i.invoiceDate).toLocaleDateString()}</span></div></td>
                  <td><span className="amount income">{fmt(i.totalAmount)}</span></td>
                  <td><span className="badge" style={{background: st.bg, color: st.color}}>{i.status}</span></td>
